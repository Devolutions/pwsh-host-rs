...
| Install | `multi-pwsh install <stable\|preview\|lts\|version\|major\|major.minor\|major.minor.x>` | `stable`, `preview`, and `lts` resolve against GitHub PowerShell releases. `major.minor.x` installs every available patch release in that line. `--root` requires explicit `--scope <user\|machine>`. |
...
| NuGet package | `Devolutions.MultiPwsh.Cli` | Package with RID-specific `multi-pwsh` native assets plus opt-in AppHost MSBuild targets that copy the binary as `multi-pwsh` or `pwsh`/`pwsh.exe` for downstream SDK outputs. |
...
| In-process Rust FFI experiment | Windows x64 package preview | `pwsh-sdk-ffi` exposes a `PATH`-resolved or explicitly selected payload `cdylib` and an SMA-free NativeAOT facade. Applications own payload provenance, integrity, and module selection. It has copied bounded sessions, copied tagged session variables, and explicit secret/credential-transfer rejection. It is experimental: one selected runtime per process, no runtime unloading or SDK binary compatibility. The package stages its native asset only when explicitly enabled. See [in-process FFI](in-process-ffi.md). |
| FFI observed invocation | Windows x64 package preview | One composite invocation carries typed results and bounded diagnostic records on independently acknowledged channels; an unacknowledged full channel backpressures the producer rather than dropping records. |
| FFI non-executing session preflight | Windows x64 package preview | `PowerShellRuntime.ValidateSessionConfiguration` resolves roots, imports, working directory, and AST-only manifest declarations without creating a runspace or importing a module. Manifests that declare module-loading paths outside an approved root are rejected. |
| FFI staged intents | Windows x64 package preview | Host-owned `stage`/`validate`/`commit`/`abort` over the existing capability dispatcher. Applications own authorization, persistence, and compensation; `Commit` acknowledges host acceptance only and promises no cross-resource atomicity. |
| FFI finite operation pages | Windows x64 package preview | `PowerShellFiniteOperationRegistry<TPage>` retains only codec-copied, fixed-schema host pages under random owner-scoped operation IDs, bounded cursor leases, deadlines, and snapshot/permission revalidation. It does not dispatch jobs, attach a bridge contract, transfer arbitrary objects, or provide application authorization or persistence. |
| FFI result projection and runtime diagnostics | Windows x64 package preview | A narrow helper projects exactly one complete result into a generated DTO with explicit cardinality/incomplete/projection failures. `PowerShellRuntime.Diagnostics` reports descriptive deployment facts only — no attestation, policy verdict, or payload objects. |
| FFI live-object contract packs | Windows x64 package preview | Consumer-to-session only, coordinated breaking releases with no version negotiation. Duplicate identifiers, direction violations, reserved identifiers, and unknown pack ABIs fail activation outright. |
| FFI duplex broker channel | Windows x64 package preview | Opt-in, strictly dispatch-only request/reply plus one-way events. A pipeline can ask the application for work without running application code on the pipeline thread. Bounded opaque byte frames and fixed-width metadata only; no callback into the consumer, CLR object, delegate, secret, or self-describing wire format. Existing `PowerShellCapabilitySet` callbacks are unchanged. See [in-process FFI](in-process-ffi.md). |
