...
| In-process Rust FFI experiment | Windows x64 package preview | `pwsh-sdk-ffi` exposes an explicit-payload `cdylib` and an SMA-free NativeAOT facade. It has copied bounded sessions, copied tagged session variables, manifest-governed configuration, and explicit secret/credential-transfer rejection. It is experimental: one selected runtime per process, no runtime unloading or SDK binary compatibility. The package stages its native asset only when explicitly enabled. See [in-process FFI](in-process-ffi.md). |
