...
| In-process Rust FFI experiment | Windows x64 package preview | `pwsh-sdk-ffi` exposes a `PATH`-resolved or explicitly selected payload `cdylib` and an SMA-free NativeAOT facade. Hash-pinned manifests are optional hardening for staged payloads and manifest-governed configuration. It has copied bounded sessions, copied tagged session variables, and explicit secret/credential-transfer rejection. It is experimental: one selected runtime per process, no runtime unloading or SDK binary compatibility. The package stages its native asset only when explicitly enabled. See [in-process FFI](in-process-ffi.md). |
